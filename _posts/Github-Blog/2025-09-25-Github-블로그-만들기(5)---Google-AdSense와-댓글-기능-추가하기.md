...
> Google 계정에 결제 프로필이 이미 등록되어 있다면 선택 후 다음 단계로 넘어가면 된다.  
{: .prompt-info }
...
**Google AdSense - 사이트** 탭에서 현재 검토 중인 사이트를 볼 수 있다. 승인에는 조건이 필요하니 관심을 가지고 꾸준히 블로그를 운영하면 이른 시일 내에 허가 받을 수 있을 것이다. 시간이 조금 지난 내용이지만 상당히 구체적인 정보를 정리해놓은 포스팅이 있다. [여기](https://wlqmffl0102.github.io/posts/Google-Adsense-Approval/)서 확인해볼 수 있다.  
...
블로그에 댓글 기능은 커뮤니티 확장에 있어 빠질 수 없는 요소이다. 다행히도 Chirpy 테마에서는 매우 간단하게 댓글 기능을 추가할 수 있도록 구성되어있다. 이전에 `_config.yml` 파일에서 다음과 같은 부분이 있었던 것을 기억이 날 것이다. 댓글 기능 관련 설정으로 이번에는 이 부분을 건드려본다.  
...
> 만약 블로그의 Repository가 Public이라면 다음 단계로 넘어가자.
{: .prompt-info }

Github의 Issue를 댓글로 사용하기 때문에 다른 사람들이 댓글을 보기 위해서 공개 저장소가 필요하다. 우리는 Private으로 생성했기 때문에 Public으로 설정된 Repository가 필요하다.  
...
가장 먼저 Github App에서 [Utterances](https://github.com/apps/utterances)를 설치한다. 우측의 **Install** 버튼을 누른다. 설치가 되어있다면 **Configure**로 보일 것이다.  
...
- `Username/Repositoryname` 양식으로 Repository를 입력한다.
...
Chirpy 테마는 간편하게 `_config_yml` 파일에서 추가할 수 있다. 아래와 같이 설정을 수정해준다. `utterances:`의 `repo` 필드는 반드시 Github의 `username/repo-name` 양식으로 입력해야한다.  

> 이전 단계인 **Utterances 설정**의 일부분(예를 들면 테마)은 Chirpy 테마에서 기본적으로 설정되어있다. 변경하고 싶다면 관련 파일을 수정하면 된다.
{: .prompt-info }
...
블로그를 배포하면 아래 이미지와 같이 댓글 기능이 추가된 것을 볼 수 있다. Github의 Issue를 활용하는 만큼 댓글을 쓰기 위해서는 Github 로그인이 필요한 점은 참고하자. 댓글 입력에는 ***Styling with Markdown is supported*** 문구로 보다시피 마크다운 문법을 허용한다. 마크다운 문법으로 작성 후 **Preview** 버튼을 눌러 작성한 댓글을 미리 확인할 수 있다.  

![이미지18](https://imgur.com/3kKwsP7.png)

누군가 댓글을 입력하면 해당 Repository의 **Issues** 탭에서도 확인할 수 있다.
